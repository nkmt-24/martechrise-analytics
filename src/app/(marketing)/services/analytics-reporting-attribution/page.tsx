...
const SLUG = 'analytics-reporting-attribution'
...
export const metadata = generateSEO({
  ...
  keywords: ['analytics reporting', 'marketing attribution', 'multi-touch attribution', 'GA4 reports', 'Looker Studio'],
})

export default function AnalyticsReportingAttributionPage() {
  return (
    <main itemScope itemType="https://schema.org/Service">
      ...
      <ServiceRelatedCaseStudies serviceSlug={SLUG} serviceTitle="Analytics Reporting & Attribution" />
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Service', name: service.meta.title, description: service.meta.description, url: `${siteConfig.url}/services/${SLUG}`, provider: { '@type': 'Organization', name: siteConfig.organization.name, url: siteConfig.url }, areaServed: [{ '@type': 'Country', name: 'United States' }, { '@type': 'Country', name: 'India' }], serviceType: 'Analytics Reporting & Attribution', hasOfferCatalog: { '@type': 'OfferCatalog', name: 'Deliverables', itemListElement: service.deliverables.items.map((item, i) => ({ '@type': 'Offer', position: i + 1, name: item.title, description: item.description })) } }) }} />
